...
| `AWS4Signer.Sign(req, cfg, metrics, ImmutableCredentials)` (exporters) | ❌ **4th param type changed** `ImmutableCredentials` → `BaseIdentity`. `ImmutableCredentials` is not a `BaseIdentity`, so current calls won't compile. Also, the `Sign(...BaseIdentity)` overload **re-resolves** credentials internally. | Resolve `ImmutableCredentials` **once** per export and sign from that exact snapshot via `AWS4Signer.SignRequest(req, cfg, null, accessKey, secretKey)` (set `request.AWS4SignerResult` + the `Authorization` header from the result). Reuse the same snapshot for the `x-amz-security-token` header so header and signature stay consistent across credential rotation. |
