...
const baseUri = 'http://dotnet-demo:8001/';
...
export default function() {
    ...
    function getHttpOutgoingCall() {
        const response = http.get(`${baseUri}outgoing-http-call/`);
        check(response, {
            [`Get outgoing-http-call response 200`] : (response) => response.status === 200
        });
    }
};