...
#include "line_probe.h" // Phase-2 line-probe PoC (AWS Distro DI) — additive
#include "pal.h"
...
private:
    ...
    // Phase-2 line-probe PoC (AWS Distro DI). Parallel preprocessor + probe store; additive.
    std::unique_ptr<LineProbeRejitPreprocessor> line_probe_preprocessor = nullptr;
    std::vector<LineProbeRequest>               line_probe_requests_;
...
public:
    ...
    // Phase-2 line-probe PoC (AWS Distro DI): same friendship as TracerMethodRewriter to reach info_.
    friend class LineProbeMethodRewriter;
};
