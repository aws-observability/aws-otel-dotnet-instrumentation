...
bool RejitHandlerModule::RemoveLineProbeByProbeId(int probeId, mdMethodDef* methodDef)
{
    std::lock_guard<std::mutex> guard(m_methods_lock);
    for (auto& kv : m_methods)
    {
        // Only line-probe handlers carry probes; the virtual hook (RTTI-free) returns nullptr for
        // CallTarget handlers, leaving them untouched.
        auto lineHandler = kv.second->AsLineProbeHandler();
...
bool RejitHandler::RemoveLineProbeAndReRejit(int probeId, const std::vector<ModuleID>& candidateModules)
{
    if (IsShutdownRequested())
    {
        return false;
    ...
    ModuleID    foundModule = 0;
    mdMethodDef foundMethod = mdTokenNil;
    {
        std::lock_guard<std::mutex> guard(m_modules_lock);
        for (ModuleID modId : candidateModules)
        {
            auto it = m_modules.find(modId);
            if (it == m_modules.end())
            {
                continue;
            }
            ...
            if (it->second->RemoveLineProbeByProbeId(probeId, &md))
            {
                foundModule = modId;
                foundMethod = md;
                break;
    ...
    if (foundMethod == mdTokenNil)
    {
        Logger::Warn("RemoveLineProbeAndReRejit: probeId ", probeId, " not found in any candidate module.");
        return false;
    }
    ...
    // exactly as EnqueueForRejit does for the add path.
    auto* info = (m_profilerInfo12 != nullptr) ? static_cast<ICorProfilerInfo7*>(m_profilerInfo12) : m_profilerInfo;
    if (info == nullptr || m_work_offloader == nullptr)
    {
        return false;
    }

    std::function<void()> action = [this, info, foundModule, foundMethod, probeId]() {
        ModuleID    mods[1]         = {foundModule};
        mdMethodDef methods[1]      = {foundMethod};
        HRESULT     revertStatus[1] = {S_OK};

        HRESULT hrRevert = info->RequestRevert(1, mods, methods, revertStatus);
        if (FAILED(hrRevert))
        {
            Logger::Warn("RemoveLineProbeAndReRejit(offloaded): RequestRevert failed hr=", HResultStr(hrRevert));
            return;
        }

        std::vector<ModuleID>    modsV{foundModule};
        std::vector<mdMethodDef> methodsV{foundMethod};
        RequestRejit(modsV, methodsV); // re-invokes the rewriter with the survivor set (or pristine)
        Logger::Info("RemoveLineProbeAndReRejit(offloaded): probeId ", probeId, " reverted + re-ReJIT'd methodDef ",
                     TokenStr(&foundMethod));
    };
    m_work_offloader->Enqueue(std::make_unique<RejitWorkItem>(std::move(action)));

    Logger::Info("RemoveLineProbeAndReRejit: removed probeId ", probeId, "; revert+rejit ENQUEUED for methodDef ",
                 TokenStr(&foundMethod), " in module ", foundModule);
    return true;
}
