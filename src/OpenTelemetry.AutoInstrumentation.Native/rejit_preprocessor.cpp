...
#include "line_probe.h" // Phase-2 line-probe PoC: instantiate RejitPreprocessor<LineProbeRequest>
...
template <class T>
static void AppendRequestToExistingMethod(RejitHandlerModule*, mdMethodDef, const T&)
{
    // no-op for non-line-probe definitions (CallTarget path is one-integration-per-method)
}

static void AppendRequestToExistingMethod(RejitHandlerModule* moduleHandler, mdMethodDef methodDef,
                                          const LineProbeRequest& request)
{
    RejitHandlerModuleMethod* existing = nullptr;
    if (moduleHandler->TryGetMethod(methodDef, &existing) && existing != nullptr)
    {
                                                                       ...
                                                                       std::vector<mdMethodDef>&        vtMethodDefs)
{
    ...
    for (; enumIterator != enumMethods.end(); enumIterator = ++enumIterator)
    {
        ...
        // per-T and the AddLineProbeRequest call only exists for the LineProbeRequest specialization).
        if (!moduleHandler->CreateMethodIfNotExists(methodDef, creator))
        {
            AppendRequestToExistingMethod(moduleHandler, methodDef, definition);
        }
...
// Phase-2 line-probe PoC (AWS Distro DI): instantiate the same template for LineProbeRequest so the
// line-probe preprocessor reuses the identical method-enumeration / ReJIT-enqueue machinery.
template class RejitPreprocessor<LineProbeRequest>;
