...
HRESULT STDMETHODCALLTYPE CorProfiler::Initialize(IUnknown* cor_profiler_info_unknown)
{
    ...
    // Phase-2 line-probe PoC (AWS Distro DI): parallel preprocessor sharing the same rejit_handler.
    line_probe_preprocessor = std::make_unique<LineProbeRejitPreprocessor>(rejit_handler, work_offloader);
...
// Phase-2 line-probe PoC (AWS Distro DI). Additive — mirrors AddInstrumentations/InternalAddInstrumentation,
// but builds LineProbeRequest records and drives the parallel line_probe_preprocessor. Touches NONE of
// the CallTarget path.
void CorProfiler::AddLineProbes(WCHAR* id, LineProbeDefinition* items, int size)
{
    auto    _             = trace::Stats::Instance()->InitializeProfilerMeasure();
    WSTRING definitionsId = WSTRING(id);
    Logger::Info("AddLineProbes: received id: ", definitionsId, " from managed side with ", size, " line probes.");

    if (size <= 0 || items == nullptr || rejit_handler == nullptr || line_probe_preprocessor == nullptr)
    {
        return;
    }
    ...
    if (definitions_ids_.find(definitionsId) != definitions_ids_.end())
    {
        Logger::Info("AddLineProbes: Id already processed.");
        return;
    }

    std::vector<LineProbeRequest> lineProbeRequests;
    for (int i = 0; i < size; i++)
    {
        const LineProbeDefinition& current = items[i];
        ...
        // Match any version of the target assembly (PoC).
        const Version& minVersion = Version(0, 0, 0, 0);
        const Version& maxVersion = Version(USHRT_MAX, USHRT_MAX, USHRT_MAX, USHRT_MAX);

        MethodReference targetMethodRef(targetAssembly, targetType, targetMethod, minVersion, maxVersion,
                                        signatureTypes);
    ...
    if (!lineProbeRequests.empty())
    {
        auto               promise = std::make_shared<std::promise<ULONG>>();
        std::future<ULONG> future  = promise->get_future();
        line_probe_preprocessor->EnqueueRequestRejitForLoadedModules(module_ids_, lineProbeRequests, promise);

        const auto status = future.wait_for(100ms);
        if (status != std::future_status::timeout)
        {
            const auto& numReJITs = future.get();
            Logger::Debug("AddLineProbes: Total number of ReJIT Requested: ", numReJITs);
        }
        else
        {
            Logger::Warn("AddLineProbes: Timeout waiting for rejit requests; rejit continues asynchronously.");
        }

        line_probe_requests_.reserve(line_probe_requests_.size() + lineProbeRequests.size());
        for (const auto& r : lineProbeRequests)
        {
            line_probe_requests_.push_back(r);
        }
    }
...
void CorProfiler::RemoveLineProbe(int probeId)
{
    ...
    std::vector<ModuleID> candidateModules;
    {
        std::scoped_lock<std::mutex> moduleLock(module_ids_lock_);
        candidateModules = module_ids_;
    }

    const bool removed = rejit_handler->RemoveLineProbeAndReRejit(probeId, candidateModules);

    // NOTE: line_probe_requests_ is a profiler-side flat AUDIT list (LineProbeRequest is not
    // copy-assignable — MethodReference has const fields — so it can't be erased in place). The
    // authoritative removal happens in the method handler (RemoveLineProbeByProbeId); the audit list
    // is not consulted for weaving, so leaving a stale entry is cosmetic. A production version would
    // switch it to a container of unique_ptr to allow erase.
    Logger::Info("RemoveLineProbe: probeId ", probeId, removed ? " removed." : " not found.");
}
