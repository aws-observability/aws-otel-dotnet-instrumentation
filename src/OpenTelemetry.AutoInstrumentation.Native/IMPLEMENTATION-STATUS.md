...
`DI_BETA_ENDPOINT` set to the non-public Application Signals beta endpoint, run from the operator's terminal (this
...
| R2 | **No branch-push CI trigger** | DECIDED, no action | Getting a signal on a push (rather than a PR) would mean giving `pr-build.yml` a `workflow_call` trigger plus `github.event_name == 'pull_request'` guards on its three PR-diff-dependent checks — i.e. changing a shared workflow to do what a draft PR already does. Use a draft PR instead. |
| R3 | **Fill the `#TBD` CHANGELOG link** | BLOCKED on the PR number | `CHANGELOG.md:21` carries a literal `#TBD` in both the link text and the URL. Must be replaced with the real PR number, and `pr-build.yml`'s static-code-checks job has a CHANGELOG check that runs on a PR. |
| R4 | **Review cycle** | NOT STARTED | A 30-commit branch touching the native profiler needs a review pass over both the .NET diff and the CI/native-build/shell changes before it goes out for wider review. |
...
| R7 | **Line-level contract test in CI** | NOT STARTED | See blocked item 6. The beta-backed proof needs credentials and can never run in CI, so an in-CI line-level contract test against the mock collector is the only automated coverage possible. The substrate is not on this branch at all: `tests/test/amazon/di`, `images/mock-di-api` and `images/applications/DynamicInstrumentation.NetCore` exist only on `feature/DI_Contract_Tests` (PR #448), so that has to land first. |
| R8 | **`pr-build.yml` has no native profiler signal** | NOT STARTED, newly identified | `pr-build.yml:151-153` runs plain `bash build.sh` and there is no `build-native-x64` equivalent — `grep build-native` in that file returns nothing. So a PR that breaks `line_probe.cpp`, drops a source file from `CMakeLists.txt`, or deletes an export goes GREEN and only breaks on main. `main-build.yml:39` has the job; `nightly-build.yml:93` and `release-build.yml:38` inherit it via `workflow_call`, so the release path is covered and the PR path is not. Note the gate at `pr-build.yml:396` `yq`-enumerates every job and treats `skipped` as failure, so a new required job must be listed there and must NOT carry a `paths:` filter. |
