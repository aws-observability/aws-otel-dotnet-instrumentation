...
{
    private const int MaxSeenTraceIds = 10_000;
    private readonly object boostLock = new();
    private long totalCount;
    private long anomalyCount;
    private long sampledAnomalyCount;
    private HashSet<string> seenTraceIds = new();
    private TraceIdRatioBasedSampler? boostedFixedRateSampler;
    ...
    public void CountTrace(string traceId)
    {
        lock (this.boostLock)
        {
            if (this.seenTraceIds.Contains(traceId))
            {
                return;
            }
    ...
    public void CountAnomalyTrace(bool isSampled)
    {
        Interlocked.Increment(ref this.anomalyCount);
        if (isSampled)
        {
            Interlocked.Increment(ref this.sampledAnomalyCount);
        }
    }

    public SamplingBoostStatisticsDocument? SnapshotBoostStatistics(string clientId, string serviceName, double timestamp)
    {
        long total;
        lock (this.boostLock)
        {
            total = Interlocked.Exchange(ref this.totalCount, 0);
            this.seenTraceIds = new HashSet<string>();
        }

        long anomaly = Interlocked.Exchange(ref this.anomalyCount, 0);
        long sampledAnomaly = Interlocked.Exchange(ref this.sampledAnomalyCount, 0);
        ...
        return new SamplingBoostStatisticsDocument(clientId, this.RuleName, serviceName, total, anomaly, sampledAnomaly, timestamp);
    }
